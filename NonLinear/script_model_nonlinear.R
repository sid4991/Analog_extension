...
analog <- function(file_path){
  ...
  df <-
    read_binary(paste0("/data/project/agaid/rajagopalan_agroecosystems/commondata/meteorologicaldata/gridded/GCMs/US_Conus/", file_path), num_vars = 8) %>%
...
plan(multicore)
models <- list.dirs("/data/project/agaid/rajagopalan_agroecosystems/commondata/meteorologicaldata/gridded/GCMs/US_Conus/", full.names = FALSE, recursive = FALSE)
#models <- models[3]
file_name <- read_lines("/data/project/agaid/rajagopalan_agroecosystems/chaudhary/AnalogData_Sid/cdl_vic_points.txt")
file_name <- read_lines("/data/project/agaid/rajagopalan_agroecosystems/chaudhary/AnalogData_Sid/cdl_vic_points.txt",skip=46000,n_max =53000)
climate_proj <- c("rcp45","rcp85")
climate_proj <- climate_proj[2]
file_path <- expand_grid(models, climate_proj, file_name) %>%
  mutate(file_path = paste(models, climate_proj, file_name, sep = "/")) %>%
  pull(file_path)
existing_file_paths <- file_path[file.exists(file.path("/data/project/agaid/rajagopalan_agroecosystems/commondata/meteorologicaldata/gridded/GCMs/US_Conus/", file_path))]
...
df <- df[-c(8:139)]
out_dir<- "/home/siddharth.chaudhary/CONUS_Model_52k_rcp_85_10_12_1/"
